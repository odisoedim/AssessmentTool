...
describe('provide/inject surveyInfo', () => {
  ...
  it('has surveyInfo from assessment and element', () => {
    const wrap = mount({
      template: `<div><span id='assessmentName'>{{info.assessmentName}}</span><span id='frameworkName'>{{info.frameworkName}}</span></div>`,
      setup() {
        provideDemoSurveyInfo(
    ...
    expectText(wrap.find('#frameworkName'), 'framework_name_1')
  })
})
